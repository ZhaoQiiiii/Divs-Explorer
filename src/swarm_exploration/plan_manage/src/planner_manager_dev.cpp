...
void FastPlannerManager::planYawActMap(const Eigen::Vector3d& start_yaw) {
  ...
  double t_traj = local_traj_.duration_;
  // search subsequent yaws
  const int seg_num = 12;
  double dt_yaw = local_traj_.duration_ / seg_num;  // time of B-spline segment
  const int subsp = 2;                              // subsampling factor to create yaw path
  double dt_path = dt_yaw * subsp;                  // time of yaw path segment
  std::cout << "duration: " << local_traj_.duration_ << ", seg_num: " << seg_num
  ...
  for (int idx = 0; idx <= seg_num; ++idx) {
    ...
    Eigen::Vector3d pd = local_traj_.position_traj_.evaluateDeBoorT(tf) - pc;
    double yc;
    if (pd.norm() > 1e-6) {
  ...
  heading_planner_->searchPathOfYaw(
      pts, spyaw, dt_path, local_traj_.position_traj_.getControlPoint(), path);
  ...
  Eigen::Vector3d end_v = local_traj_.velocity_traj_.evaluateDeBoorT(local_traj_.duration_ - 0.1);
...
bool FastPlannerManager::localExplore(Eigen::Vector3d start, Eigen::Vector3d start_vel,
  ...
  local_traj_.start_time_ = ros::Time::now();
  ...
  std::cout << "Local explore time: " << (ros::Time::now() - local_traj_.start_time_).toSec()
            << std::endl;

  local_traj_.position_traj_.setUniformBspline(ctrl_pts, 3, dt);
