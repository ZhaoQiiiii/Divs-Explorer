...
void FastPlannerManager::initPlanModules(ros::NodeHandle& nh) 
{
  ...
  local_traj_.traj_id_ = 0;
  guide_traj_.traj_id_ = 0;
...
// Waypoints-based
void FastPlannerManager::planExploreTraj(const vector<Eigen::Vector3d>& tour, const Eigen::Vector3d& cur_vel, 
  const Eigen::Vector3d& cur_acc, const double& time_lb) 
{
  // Astar + B-spline Optimization

  if (tour.empty()) ROS_ERROR("Empty path to traj planner");

  // 
  // 1、Get the Polynomial Trajectory from A* Path
  //
  ...
  // Set the times vector by Trapezoid Time Allocator
  ...
  // Get the NonUniform B-spline with segment time, waypoints, derivatives and degree
  ...
  //
  // 4、Start Optimize to get the Uniform B-spline
  //

  bspline_optimizers_[0]->optimize(ctrl_pts, dt, cost_func, 1, 1);
  local_traj_.position_traj_.setUniformBspline(ctrl_pts, pp_.bspline_degree_, dt);
  updateMotionTrajInfo();
}

void FastPlannerManager::planGuideTraj(const vector<Eigen::Vector3d>& tour, const Eigen::Vector3d& cur_vel, 
                                       const Eigen::Vector3d& cur_acc, const double& time_lb)
{
  // Astar + B-spline Optimization
  ...
  // Get the Initial Points from A* for Polynomial Traj
  const int pt_num = tour.size();
  Eigen::MatrixXd pos(pt_num, 3);
  for (int i = 0; i < pt_num; ++i) { 
    pos.row(i) = tour[i];
  ...
  // Set the Times vector with Trapezoid Time Allocator
  Eigen::Vector3d zero(0, 0, 0);
  Eigen::VectorXd times(pt_num - 1);
  for (int i = 0; i < pt_num - 1; ++i) {
    times(i) = (pos.row(i + 1) - pos.row(i)).norm() / (pp_.max_vel_ * 0.5);
  ...
  //
  // 2、Parameterize the Polynomial Trajectory to NonUniform B-spline
  //
  ...
  // Get the Ctrl Points of NonUniform B-spline with Segment Time, Waypoints, Derivatives and Degree
  Eigen::MatrixXd ctrl_pts;
  NonUniformBspline::parameterizeToBspline(dt, points, boundary_deri, pp_.bspline_degree_, ctrl_pts);

  // Parameterize to NonUniform B-spline with Ctrl Points
  NonUniformBspline tmp_traj(ctrl_pts, pp_.bspline_degree_, dt);


  //
  // 3、Set the Params of B-spline Optimization
  //
  ...
  //
  // 4、Optimize the Ctrl Points and Get the Uniform B-spline
  //

  bspline_optimizers_[0]->optimize(ctrl_pts, dt, cost_func, 1, 1);
  guide_traj_.position_traj_.setUniformBspline(ctrl_pts, pp_.bspline_degree_, dt);
  updateGuideTrajInfo();
}
...
{
  // Kino Astar + B-spline Optimization 
  ...
  local_traj_.position_traj_.setUniformBspline(ctrl_pts, pp_.bspline_degree_, ts);

  vector<Eigen::Vector3d> start2, end2;
  local_traj_.position_traj_.getBoundaryStates(2, 0, start2, end2);
...
void FastPlannerManager::planYaw(const Eigen::Vector3d& start_yaw) 
{
  ...
  auto& pos = local_traj_.position_traj_;
  ...
  Eigen::Vector3d end_v = local_traj_.velocity_traj_.evaluateDeBoorT(duration - 0.1);
  ...
  // update traj info
  local_traj_.yaw_traj_.setUniformBspline(yaw, pp_.bspline_degree_, dt_yaw);
  local_traj_.yawdot_traj_ = local_traj_.yaw_traj_.getDerivative();
  local_traj_.yawdotdot_traj_ = local_traj_.yawdot_traj_.getDerivative();
...
void FastPlannerManager::planExploreYaw(const Eigen::Vector3d& start_yaw, const double& end_yaw,
                                        bool lookfwd, const double& relax_time) 
{
  // Only consider smoothness and boundary state

  const int seg_num = 12;
  double dt_yaw = local_traj_.duration_ / double(seg_num);  // time of B-spline segment
  ...
  if (lookfwd) 
  {
    ...
    for (int i = 1; i < seg_num - relax_num2; ++i) {
      double tc = i * dt_yaw;
      Eigen::Vector3d pc = local_traj_.position_traj_.evaluateDeBoorT(tc);
      double tf = min(local_traj_.duration_, tc + forward_t);
      Eigen::Vector3d pf = local_traj_.position_traj_.evaluateDeBoorT(tf);
      Eigen::Vector3d pd = pf - pc;
      Eigen::Vector3d waypt;
      if (pd.norm() > 1e-6) {
        waypt(0) = atan2(pd(1), pd(0));
  ...
  // Update traj info
  local_traj_.yaw_traj_.setUniformBspline(yaw, 3, dt_yaw);
  local_traj_.yawdot_traj_ = local_traj_.yaw_traj_.getDerivative();
  local_traj_.yawdotdot_traj_ = local_traj_.yawdot_traj_.getDerivative();
  plan_data_.dt_yaw_ = dt_yaw;
...
void FastPlannerManager::planGuideYaw(const Eigen::Vector3d& start_yaw) 
{
  auto t1 = ros::Time::now();
  // calculate waypoints of heading

  auto& pos = guide_traj_.position_traj_;
  double duration = pos.getTimeSum();
  ...
  Eigen::Vector3d end_v = guide_traj_.velocity_traj_.evaluateDeBoorT(duration - 0.1);
  Eigen::Vector3d end_yaw(atan2(end_v(1), end_v(0)), 0, 0);
  calcNextYaw(last_yaw, end_yaw(0));
  yaw.block(seg_num, 0, 3, 1) = states2pts * end_yaw;
  ...
  // update traj info
  guide_traj_.yaw_traj_.setUniformBspline(yaw, pp_.bspline_degree_, dt_yaw);
  guide_traj_.yawdot_traj_ = guide_traj_.yaw_traj_.getDerivative();
  guide_traj_.yawdotdot_traj_ = guide_traj_.yawdot_traj_.getDerivative();
...
bool FastPlannerManager::topoReplan(bool collide) {
  ...
  if (!collide) {
    ...
    local_traj_.position_traj_ = init_traj;
    global_data_.setLocalTraj(
        local_traj_.position_traj_, t_now, local_traj_duration + time_change + t_now, time_change);
    ...
    if (colli_start.size() == 1 && colli_end.size() == 0) {
      ...
      local_traj_.position_traj_ = best_traj;
      global_data_.setLocalTraj(local_traj_.position_traj_, t_now,
...
bool FastPlannerManager::planGlobalTraj(const Eigen::Vector3d& start_pos) {
  ...
  local_traj_.position_traj_ = bspline;
  local_traj_.start_time_ = time_now;
...
bool FastPlannerManager::checkTrajCollision(double& distance) {
  ...
  double t_now = (ros::Time::now() - local_traj_.start_time_).toSec();
  Eigen::Vector3d cur_pt = local_traj_.position_traj_.evaluateDeBoorT(t_now);
  Eigen::Vector3d fut_pt;

  double radius = 0.0;
  double fut_t = 0.02;

  while (radius < 6.0 && t_now + fut_t < local_traj_.duration_) {
    fut_pt = local_traj_.position_traj_.evaluateDeBoorT(t_now + fut_t);
    // double dist = edt_environment_->sdf_map_->getDistance(fut_pt);

    if (sdf_map_->getInflateOccupancy(fut_pt) == 1) {
      distance = radius;
      // ROS_WARN_STREAM("[Safety] Collision at: " << fut_pt.transpose());
      return false;
    }
...
void FastPlannerManager::updateMotionTrajInfo() {
...
void FastPlannerManager::updateGuideTrajInfo() {
  guide_traj_.velocity_traj_ = guide_traj_.position_traj_.getDerivative();
  guide_traj_.acceleration_traj_ = guide_traj_.velocity_traj_.getDerivative();

  guide_traj_.start_pos_ = guide_traj_.position_traj_.evaluateDeBoorT(0.0);
  guide_traj_.duration_ = guide_traj_.position_traj_.getTimeSum();

  guide_traj_.traj_id_ += 1;
}
...
//
void FastPlannerManager::optimizeTopoBspline(double start_t, double duration, vector<Eigen::Vector3d> guide_path, int traj_id) {
  auto t1 = ros::Time::now();
...
//
Eigen::MatrixXd FastPlannerManager::reparamLocalTraj(const double& start_t, const double& duration, const double& dt) 
{
  vector<Eigen::Vector3d> point_set;
  vector<Eigen::Vector3d> start_end_derivative;
...
//
void FastPlannerManager::findCollisionRange(vector<Eigen::Vector3d>& colli_start, vector<Eigen::Vector3d>& colli_end, 
...
bool FastPlannerManager::checkSwarmCollision(const int& id) {
  ...
  double self_t = 0.02 + (ros::Time::now() - local_traj_.start_time_).toSec();
  double other_t = 0.02 + ros::Time::now().toSec() - traj.start_time_;

  while (self_t < local_traj_.duration_ && self_t > 0 && other_t < traj.duration_) {
    auto self_pos = local_traj_.position_traj_.evaluateDeBoorT(self_t);
