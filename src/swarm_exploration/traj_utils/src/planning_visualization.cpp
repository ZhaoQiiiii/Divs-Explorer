...
PlanningVisualization::PlanningVisualization(ros::NodeHandle& nh) 
{
  ...
  updated_box_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/updated_box", 100);
  pubs_.push_back(updated_box_pub_);

  frontier_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/frontier", 10000);
  pubs_.push_back(frontier_pub_);

  viewpoint_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/viewpoint", 1000);
  pubs_.push_back(viewpoint_pub_);

  division_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/division", 1000);
  pubs_.push_back(division_pub_);

  tour_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/tour", 100);
  pubs_.push_back(tour_pub_);

  racer_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/racer", 100);
  pubs_.push_back(racer_pub_);
...
// Text
void PlanningVisualization::drawText(const Eigen::Vector3d& pos, const string& text, const double& scale, 
                                     const Eigen::Vector4d& color, const string& ns, const int& id, const int& pub_id) 
{
  visualization_msgs::Marker mk;
...
// Sphere
void PlanningVisualization::drawSphere(const Eigen::Vector3d& point, const double& scale, const Eigen::Vector4d& color, 
...
// Cube
void PlanningVisualization::drawCube(const Eigen::Vector3d& point, const Eigen::Vector3d& scale,
                                     const Eigen::Vector4d& color, const string& ns, const int& id,
                                     const int& pub_id) 
{
  visualization_msgs::Marker mk;
  fillBasicInfo(mk, scale, color, ns, id, visualization_msgs::Marker::CUBE_LIST);
...
void PlanningVisualization::fillGeometryInfo(visualization_msgs::Marker& mk, const Eigen::Vector3d& point) 
{
...
void PlanningVisualization::drawBspline(NonUniformBspline& bspline, double size,
    const Eigen::Vector4d& color, bool show_ctrl_pts, double size2, const Eigen::Vector4d& color2, int id1) 
{
