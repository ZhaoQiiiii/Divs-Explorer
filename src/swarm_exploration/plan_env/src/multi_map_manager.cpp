...
void MultiMapManager::init() {
  ...
  marker_pub_ = nh_.advertise<visualization_msgs::Marker>("/multi_map_manager/marker_" + std::to_string(drone_id_), 10);

  stamp_sub_ = nh_.subscribe("/multi_map_manager/chunk_stamps_recv", 10, &MultiMapManager::stampMsgCallback, this);
  chunk_sub_ = nh_.subscribe("/multi_map_manager/chunk_data_recv", 5000, &MultiMapManager::chunkCallback, this, ros::TransportHints().tcpNoDelay());

  multi_map_chunks_.resize(map_num_);
  for (auto& data : multi_map_chunks_) data.idx_list_ = {};

  chunk_buffer_.resize(map_num_);
  buffer_map_.resize(map_num_);

  chunk_boxes_.resize(map_num_);
  for (auto& box : chunk_boxes_) box.valid_ = false;
...
// Update the Map Chunk after get New Voxels from other robots (sdf_map.cpp)
...
// Get the Map Chunk Boxes (frontier_finder.cpp and uniform_grid.cpp)
void MultiMapManager::getChunkBoxes(vector<Eigen::Vector3d>& mins, vector<Eigen::Vector3d>& maxs, bool reset) 
{
...
void MultiMapManager::chunkTimerCallback(const ros::TimerEvent& e) {
  // Update the Chunks Data for each drone by processing chunks in buffers 
  ...
  for (int i = 0; i < chunk_buffer_.size(); ++i) {
    ...
    sort(buffer.begin(), buffer.end(), [](const plan_env::ChunkData& chunk1, const plan_env::ChunkData& chunk2) {
...
void MultiMapManager::stampTimerCallback(const ros::TimerEvent& e) {
  // Send time stamp of chunks to other drones
  ...
  stamp_pub_.publish(msg); // "/multi_map_manager/chunk_stamps"
  return;
...
// Sub "/multi_map_manager/chunk_data"
void MultiMapManager::chunkCallback(const plan_env::ChunkDataConstPtr& msg) {
  ...
  if (buffer_map_[msg->chunk_drone_id - 1].find(msg->idx) !=
      buffer_map_[msg->chunk_drone_id - 1].end()) return;
...
// Sub "/multi_map_manager/chunk_stamps"
void MultiMapManager::stampMsgCallback(const plan_env::ChunkStampsConstPtr& msg) {
  // Receive chunks stamps from other drones, store them in last time stamp 

  // Check msg valid
  if (msg->from_drone_id == drone_id_) return;
  if (drone_id_ == map_num_) return; // Ground node does not send chunk
  if (msg->time - last_chunk_stamp_time_[msg->from_drone_id - 1] < 0.3) return; // Avoid overwhelming 
...
void MultiMapManager::findMissedChunkIds(const vector<int>& self_idx_list, const vector<int>& other_idx_list, vector<int>& miss_ids) {
...
void MultiMapManager::sendChunks(const int& chunk_drone_id, const int& to_drone_id, const vector<int>& idx_list) {
  ...
  for (int i = 0; i < idx_list.size(); i += 2) {
    for (int j = idx_list[i]; j <= idx_list[i + 1]; ++j) {
      ...
      chunk_pub_.publish(msg); // "/multi_map_manager/chunk_data"
...
bool MultiMapManager::findIntersect(const int& min1, const int& max1, const int& min2, const int max2, int& minr, int& maxr) {
  minr = max(min1, min2);
  maxr = min(max1, max2);
...
void MultiMapManager::mergeChunkIds(const vector<int>& list1, const vector<int>& list2, vector<int>& output) {
