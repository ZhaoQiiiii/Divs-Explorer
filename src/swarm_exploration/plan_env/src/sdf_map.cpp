...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  // Get the Index of Origin (For Ground)
  Eigen::Vector3d origin = {0, 0, 0};
  posToIndex(origin, mp_->index_origin_);
...
void SDFMap::inputPointCloud(const pcl::PointCloud<pcl::PointXYZ>& points, const int& point_num, 
  ...
  for (int i = 0; i < point_num; ++i) {
    ...
    // Truncate the point cloud with max_ray_length_
    int occ_flag;
    if (!isInMap(pt_w)) {
      pt_w = closetPointInMap(pt_w, sensor_pos);
      length = (pt_w - sensor_pos).norm();
      if (length > mp_->max_ray_length_) pt_w = (pt_w - sensor_pos) / length * mp_->max_ray_length_ + sensor_pos;
    ...
    setCacheOccupancy(vox_adr, occ_flag);

    // Set Free Voxels between sensor pos and point by raycaster
    if (md_->flag_rayend_[vox_adr] == md_->raycast_num_) continue;
    else md_->flag_rayend_[vox_adr] = md_->raycast_num_;
  ...
  //
  // 3、Get the Updated Box
  //

  // Inflate to get Local Updated Box
  ...
  mr_->local_updated_ = true; // Allow to update Local Map
  ...
  // for (int k = 0; k < 3; ++k) {
  //   md_->update_min_[k] = max(min(update_min[k], md_->update_min_[k]), mp_->box_mind_[k]);
  //   md_->update_max_[k] = min(max(update_max[k], md_->update_max_[k]), mp_->box_maxd_[k]);
  //   md_->all_min_[k] = max(min(update_min[k], md_->all_min_[k]), mp_->box_mind_[k]);
  //   md_->all_max_[k] = min(max(update_max[k], md_->all_max_[k]),mp_->box_maxd_[k]);
  // }
  ...
  while (!md_->cache_voxel_.empty()) {
    ...
    md_->occupancy_buffer_[adr] = std::min(std::max(md_->occupancy_buffer_[adr] + log_odds_update, mp_->clamp_min_log_), 
                                           mp_->clamp_max_log_);
  }
...
void SDFMap::InflateLocalMap() {
  ...
  // Inflate the newest occpuied voxels
...
void SDFMap::setCacheOccupancy(const int& adr, const int& occ) {
  ...
  if (occ == 0) md_->count_miss_[adr] = 1;
  else md_->count_hit_[adr] += 1; // if (occ == 1) 
    
  // md_->count_hit_and_miss_[adr] += 1;
  // if (occ == 1) md_->count_hit_[adr] += 1;
  // if (md_->count_hit_and_miss_[adr] == 1) md_->cache_voxel_.push(adr);
}
