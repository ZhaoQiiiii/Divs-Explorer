...
struct MapParam 
{
  ...
  Eigen::Vector3i index_origin_; // Origin (0, 0, 0) Index (For Ground)

  // Map Fusion
  double p_hit_, p_miss_, p_min_, p_max_, p_occ_;  // occupancy probability
  double prob_hit_log_, prob_miss_log_, clamp_min_log_, clamp_max_log_;
  double min_occupancy_log_;  // Logit Threshold of Free and Unknown 
...
{
  // Main
  std::vector<double> occupancy_buffer_;        // Occupied Probability of Voxels
  std::vector<char> occupancy_buffer_inflate_;  // Occupied Flag of Inflated Voxels 

  std::vector<double> distance_buffer_neg_;     // Euclidean negative distance
  std::vector<double> distance_buffer_;         // Euclidean distance
  ...
  Eigen::Vector3d update_min_, update_max_;             // Updated Box for single  
  Eigen::Vector3d all_min_, all_max_;                   // Overall Updated Box for multi
  Eigen::Vector3i local_bound_min_, local_bound_max_;   // Local Updated Box (Inflated)
