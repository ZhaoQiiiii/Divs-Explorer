...
// Hierarchical Grid (Hgrid = Grid Lv1 + Grid Lv2)
class HGrid {
...
public:
  ...
  void updateGridsData(const int& drone_id, vector<int>& grid_ids, bool reallocated,
                      const vector<int>& last_grid_ids, vector<int>& first_ids, vector<int>& second_ids);
  void getCostMatrix(const vector<Eigen::Vector3d>& positions, const vector<Eigen::Vector3d>& velocities, 
  ...
  double getCostDroneToGrid(const Eigen::Vector3d& pos, const int& grid_id, const vector<int>& first);
...
private:
  ...
  // Two-Level Grids
  unique_ptr<UniformGrid> grid1_;  // one-level (Coarse)
  unique_ptr<UniformGrid> grid2_;  // two-level (Fine)
