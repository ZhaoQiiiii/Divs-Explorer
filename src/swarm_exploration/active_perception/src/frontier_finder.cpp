...
void FrontierFinder::computeFrontiersToVisit() {
  // Find viewpoints (x,y,z,yaw) for all clusters and Find the informative ones
  ...
  for (auto& tmp_ftr : tmp_frontiers_) {
    ...
    if (!tmp_ftr.vps_.empty()) {
      ...
      sort(inserted->vps_.begin(), inserted->vps_.end(),
...
void FrontierFinder::updateFrontierCostMatrix() {
  ...
  auto updateCost = [](const list<Frontier>::iterator& it1, const list<Frontier>::iterator& it2) {
    ...
    Viewpoint& vui = it1->vps_.front();
    Viewpoint& vuj = it2->vps_.front();
    vector<Vector3d> path_ij;
    double cost_ij = ViewNode::computeCostLB(
...
// Get the matrix of Asymmetric TSP  
void FrontierFinder::getFullCostMatrix(const Vector3d& cur_pos, const Vector3d& cur_vel, 
  ...
  // 
  // 1、Fill block with cost between clusters
  // 

  int i = 1, j = 1;
  for (auto frontier : frontiers_) {
    for (auto cost : frontier.costs_) {
      // std::cout << "(" << i << ", " << j << ")"
      // << ", ";
      mat(i, j++) = cost;
    }
  ...
  // 
  // 2、Fill block with cost from current state to clusters
  //
  ...
  for (auto frontier : frontiers_) {
    // std::cout << "(0, " << j << ")"
    // << ", ";
    Viewpoint vj = frontier.vps_.front();
    vector<Vector3d> path;
    mat(0, j++) = ViewNode::computeCostLB(cur_pos, vj.pos_, cur_yaw[0], vj.yaw_, cur_vel, cur_yaw[1], path);
  }
...
void FrontierFinder::getViewpointsInfo(const Vector3d& cur_pos, const vector<int>& ids, const int& view_num, 
  const double& max_decay, vector<vector<Eigen::Vector3d>>& points, vector<vector<double>>& yaws) {
  points.clear();
  yaws.clear();
  for (auto id : ids) {
    ...
    for (auto frontier : frontiers_) {
      if (frontier.id_ == id) {
        ...
        vector<double> ys;
        int visib_thresh = frontier.vps_.front().visib_num_ * max_decay;
        for (auto view : frontier.vps_) {
...
void FrontierFinder::sampleViewpoints(Frontier& frontier) {
  ...
  for (double rc = candidate_rmin_, dr = (candidate_rmax_ - candidate_rmin_) / candidate_rnum_;
    ...
    for (double phi = -M_PI; phi < M_PI; phi += candidate_dphi_) {
      ...
      if (!edt_env_->sdf_map_->isInBox(sample_pos) || isNearUnknown(sample_pos) ||
          edt_env_->sdf_map_->getInflateOccupancy(sample_pos) == 1) continue;
...
// Get the Cost Matrix of Exploration Path
void FrontierFinder::getSwarmCostMatrix(const vector<Vector3d>& positions, const vector<Vector3d>& velocities, 
  const vector<double>& yaws, const vector<int>& ftr_ids, const vector<Eigen::Vector3d>& grid_pos, Eigen::MatrixXd& mat) {
  
  //
  // 1、Get the initital cost matrix with (Drones, Frontiers and Virtual Depot) (As the temp data)
  //
  ...
  //
  // 2、Get the full cost matrix with (Drones, Selected Frontiers, Virtual Depot and Next Grid)
  //
  ...
  // Costs from Virtual depot to drones
  for (int i = 0; i < drone_num; ++i) {
    mat(0, 1 + i) = -1000;
    mat(1 + i, 0) = 1000;
  }

  // Costs from Virtual depot to frontiers
  for (int i = 0; i < ftr_num; ++i) {
    mat(0, 1 + drone_num + i) = 1000;
    mat(1 + drone_num + i, 0) = 0;
  }

  // Costs from Costs between drones
  for (int i = 0; i < drone_num; ++i) {
  ...
  if (!grid_pos.empty()) {
    ...
    for (int i = 0; i < ftr_num; ++i) {
      double cost = ViewNode::computeCostLB(
...
void FrontierFinder::getSwarmCostMatrix(const vector<Vector3d>& positions,
  ...
  for (int i = 0; i < drone_num; ++i) {
    int j = 0;
    for (auto ftr : frontiers_) {
      Viewpoint vj = ftr.vps_.front();
      vector<Vector3d> path;
      mat(1 + i, 1 + drone_num + j) =
          ViewNode::computeCostLB(positions[i], vj.pos_, yaws[0], vj.yaw_, velocities[i], 0.0, path);
      mat(1 + drone_num + j, 1 + i) = 0;
      ++j;
    }
  }
...
// Get the Vis info of Global Tour
void FrontierFinder::getPathForTour(const Vector3d& pos, const vector<int>& frontier_ids, vector<Vector3d>& path) {
  ...
  ViewNode::searchPath(pos, frontier_indexer[frontier_ids[0]]->vps_.front().pos_, segment);
