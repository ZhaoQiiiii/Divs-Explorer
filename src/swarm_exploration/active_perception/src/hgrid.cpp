...
// Input Frontiers into Hgrid's both levels
void HGrid::inputFrontiers(const vector<Eigen::Vector3d>& avgs) {
  grid1_->inputFrontiers(avgs); 
  grid2_->inputFrontiers(avgs);
}

// Update the HGrid Info + Allocate HGrid IDs for single drone
void HGrid::updateGridsData(const int& drone_id, vector<int>& grid_ids, bool reallocated, 
  const vector<int>& last_grid_ids, vector<int>& first_ids, vector<int>& second_ids) {

  //
  // 1、Get the IDs of Hgrid contained by single drone 
  //

  vector<int> grid_ids1, grid_ids2;
  const int grid_num1 = grid1_->grid_data_.size(); 
  for (auto id : grid_ids) {
    if (id < grid_num1) grid_ids1.push_back(id); 
    else grid_ids2.push_back(id - grid_num1);
  }


  //
  // 2、Update the Grid Lv1 for ego-drone
  //

  vector<int> parti_ids1, parti_ids1_all; // Should not be Empty
  grid1_->updateGridsData(drone_id, grid_ids1, parti_ids1, parti_ids1_all);
  ...
  // Convert all Coarse Grid to Fine Grid 
  ...
  grid2_->activateGrids(fine_ids_all); // Activate newly divided grids to Vis

  // if (reallocated) grid2_->activateGrids(grid_ids2);


  //
  // 3、Update the Grid Lv2 for ego-drone
  //

  vector<int> parti_ids2, parti_ids2_all; // Should be Empty
  grid2_->updateGridsData(drone_id, grid_ids2, parti_ids2, parti_ids2_all);


  //
  // 4、Get the Allocated HGrid for ego-drone
  //

  // HGrid = Grid Lv1 + Grid Lv2 (Allocated)
  grid_ids = grid_ids1;
  for (auto& id : grid_ids2) grid_ids.push_back(id + grid_num1);
...
// Get the Cost Matrix of Coverage Path
void HGrid::getCostMatrix(const vector<Eigen::Vector3d>& positions, const vector<Eigen::Vector3d>& velocities, 
  const vector<vector<int>>& first_ids, const vector<vector<int>>& second_ids, const vector<int>& grid_ids, Eigen::MatrixXd& mat) {
...
// Update the Base Coordinate and Basic Info for Hgrid
